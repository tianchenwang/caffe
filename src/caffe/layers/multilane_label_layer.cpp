...
void MultilaneLabelLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  const bool depth_only = this->layer_param_.multilane_label_param().depth_only();
  const int grid_dim = this->layer_param_.multilane_label_param().grid_dim();
  const int blob_depth = depth_only? (grid_dim*grid_dim)*2:(grid_dim*grid_dim)*7;
  if (crop_size > 0) {
...
void MultilaneLabelLayer<Dtype>::InternalThreadEntry() {
  ...
  if(Caffe::mpi()->rank()==0)
    LOG(INFO)<<"Rank "<<Caffe::mpi()->rank()<<"/"<<Caffe::mpi()->size()<<": reading label batch "<<lines_id_<<", "<<filename;

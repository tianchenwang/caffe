...
void LRNFixedLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void LRNFixedLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
  num_ = bottom[0]->num();
...
template <typename Dtype>
void LRNFixedLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    vector<Blob<Dtype>*>* top) {
  switch (this->layer_param_.lrn_param().norm_region()) {
  case LRNParameter_NormRegion_ACROSS_CHANNELS:
...
template <typename Dtype>
void LRNFixedLayer<Dtype>::CrossChannelForward_cpu(
...
template <typename Dtype>
void LRNFixedLayer<Dtype>::WithinChannelForward(
    const vector<Blob<Dtype>*>& bottom, vector<Blob<Dtype>*>* top) {
...
template <typename Dtype>
void LRNFixedLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
template <typename Dtype>
void LRNFixedLayer<Dtype>::CrossChannelBackward_cpu(
...
template <typename Dtype>
void LRNFixedLayer<Dtype>::WithinChannelBackward(
    const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down,
    vector<Blob<Dtype>*>* bottom) {
  if (propagate_down[0]) {
...
#ifdef CPU_ONLY
STUB_GPU(LRNFixedLayer);
STUB_GPU_FORWARD(LRNFixedLayer, CrossChannelForward);
STUB_GPU_BACKWARD(LRNFixedLayer, CrossChannelBackward);
#endif

...
template <typename Dtype>
cv::Mat VideoDataLayer<Dtype>::ReadVideoFrameToCVMat(const string& filename, size_t id, size_t persp,
    const int height, const int width) {
  cv::Mat cv_img, cv_img_origin;
  bool set_ok = this->cap->set(CV_CAP_PROP_POS_FRAMES, id );
  if(!set_ok) {
    LOG(ERROR)<<"Failed to set video frame of "<<filename; 
  }
  bool read_ok = this->cap->read(cv_img_origin);
  if(!read_ok) {
    LOG(ERROR)<<"Failed to read video frame from "<<filename;
  }
  ...
  cv::cvtColor(cv_img, cv_img, CV_BGR2RGB); // convert to RGB
  return cv_img; 
}
...
void VideoDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->layer_param_.video_data_param().rand_skip()) {
    ...
    lines_id_ = skip + Caffe::mpi()->rank();
  }
  // Read a data batch, and use it to initialize the top blob.
  this->cap = new cv::VideoCapture(lines_[lines_id_].first);
  cv::Mat cv_img = ReadVideoFrameToCVMat(lines_[lines_id_].first, lines_[lines_id_].second.first[0],
                         lines_[lines_id_].second.second[0], new_height, new_width); 
...
void VideoDataLayer<Dtype>::InternalThreadEntry() {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    // get a blob
    timer.Start();
    cv::Mat cv_img = ReadVideoFrameToCVMat(filename, frameIds[item_id], trans[item_id],
          new_height, new_width);
    CHECK(cv_img.data) << "Could not read frame "<<frameIds[item_id]<<" from " << filename; 
